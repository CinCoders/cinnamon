...
- **`Footer`**: hard-coded hex colors replaced with design token CSS vars (`bg-cinnamon-footer-bg`, `bg-cinnamon-footer-bar`). `phoneToTel()` helper moved to `src/lib/utils.ts`. `copyrightText` now has a default, so omitting it shows `CIn UFPE | Todos os direitos reservados` rather than hiding the line as in v1; pass `null` for the v1 behaviour.
