 ...
 * - Não usar para autorização real no server
 * - Une realm roles (realm_access.roles) com client roles (resource_access[*].roles).
 *   Passe `clientId` para restringir as client roles a um único client.
 */
export function unsafeDecodeRolesFromKeycloakAccessToken(
  accessToken: string,
  clientId?: string,
): string[] {
  ...
  const resourceRoles = clientId
    ...
    : Object.values(resourceAccess).flatMap((client) => client.roles ?? []);
...
//
// ATENÇÃO: usa unsafeDecode* (sem validar assinatura). É aceitável no client
// (a UI não é fronteira de segurança). No server, monte a CinnamonSession a
// partir de um token já validado (jose/introspection) — o entry `./server` não
// expõe este helper justamente por isso.
export function sessionFromOidcAuth(
  auth: OidcAuthLike,
  clientId?: string,
): CinnamonSession {
  if (!auth.user) {
  ...
  return {
    ...
    roles: unsafeDecodeRolesFromKeycloakAccessToken(auth.user.access_token, clientId),
    user: {
      id: profile?.sub,
      email: profile?.email,
      name: profile?.name,
